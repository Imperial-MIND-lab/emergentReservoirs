function [EC_mat, EC_density, etem] = structured_G_causality_al857(timeseries, DTI_mat,  normalisation_option);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
[brain_par, num_timepoints] = size(timeseries); %90; % 90 because we are used the main part
...
%addpath('libs'); %al857 this should be redundant
...
switch normalisation_option
    ...
    case 2
        ...
        max_val = max(max(DTI_mat));
        DTI_mat = DTI_mat/max_val;
    otherwise
        disp('No normalization of the initial matrix')
...
while  (  error_des > th  )  && ( iter < maxiter )
        ...
        for jj = 1 : num_timepoints -1
             error_des  = error_des +   norm(  DTI_mat * averaged_bold_seq(jj,:)'- averaged_bold_seq(jj+1,:)' , 2 ) ;
        end
        ...
        for jj = 1 : num_timepoints -1
            gradient = gradient + ( ( DTI_mat*averaged_bold_seq(jj,:)'*averaged_bold_seq(jj,:))  - (averaged_bold_seq(jj+1,:)'*averaged_bold_seq(jj,:)) ); 
        ...
        %gradient
        DTI_mat =  DTI_mat - eta * gradient;
        
        %Reinforce where there was no connection at the beginning
        DTI_mat = DTI_mat .* noconn_M;
        %Remove negative values
        DTI_mat = DTI_mat .*( DTI_mat>0);
        iter = iter + 1 ;
end 
...
EC_mat = DTI_mat;
EC_density = numel(find(EC_mat >0)) ./ numel(EC_mat);
