function [R,eff] = randmio_und_connected_hmt(R, ITER)
%RANDMIO_UND_CONNECTED     Random graph with preserved degree distribution
% modified by hmt to enable rewiring ITER edges (not each edge ITER times).
%
...
maxAttempts = 100*ITER;
% actual number of successful rewirings
eff = 0;


att=0;
while (eff<ITER) && (att<=maxAttempts)                                   %while not rewired
    rewire=1;
    % pick two edges connecting 4 different vertices
    while 1
        e1=ceil(K*rand);
        e2=ceil(K*rand);
        while (e2==e1)
